...
const MAP_ASPECT_RATIO = TILED_MAP_WIDTH / TILED_MAP_HEIGHT;
...
export default function Sunnybrook() {
  ...
  const goToClinic = () => {
    void navigate("/sunnybrook/SBClinic");
  };
  ...
  useEffect(() => {
    ...
    const fetchMapData = async () => {
      ...
      try {
        ...
        if (!raw || typeof raw !== 'object' || !('layers' in raw)) {
          throw new Error('Invalid map data format');
        }
        const tiledMapData = raw as TiledMapData;
        ...
        if (hotspotLayer && hotspotLayer.objects) {
          ...
          const processedHotspots: AppHotspot[] = hotspotLayer.objects.map((obj: TiledObject) => {
            ...
            const type = getTiledObjectProperty(obj, 'type') || 'location';
  ...
  const handleHotspotClick = (hotspot: AppHotspot) => {
    void navigate(hotspot.route || '/sunnybrook');
  };
  ...
  if (isLoading) {
    return (
      <div className="sunnybrook-status-message sunnybrook-loading">
        ...
        <br />
        <button onClick={forceLoad} style={{ marginTop: '20px', padding: '10px', cursor: 'pointer' }}>
          Skip Loading
        </button>
      </div>
    );
  ...
  return (
    <div className="sunnybrook-page">
      <button onClick={toggleDebug} style={{ position: 'fixed', top: '10px', right: '10px', zIndex: 1000, padding: '5px 10px', backgroundColor: 'rgba(0,0,0,0.5)', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>
      ...
      <div className="map-container">
        ...
        <div className="map-content" style={{ width: `${mapDimensions.width}px`, height: `${mapDimensions.height}px`, backgroundImage: `url(${SUNNYBROOK_MAP_URL})` }}>
          <div onClick={goToClinic} style={{ position: 'absolute', left: '675px', top: '445px', width: '60px', height: '60px', transform: `translate(-50%, -50%) scale(${scale})`, backgroundColor: showClickableAreas ? 'rgba(255, 0, 0, 0.3)' : 'transparent', border: showClickableAreas ? '2px solid white' : 'none', borderRadius: '50%', cursor: 'pointer', zIndex: 999 }} />
          <MapCanvas hotspots={hotspots} canvasWidth={mapDimensions.width} canvasHeight={mapDimensions.height} onHotspotClick={handleHotspotClick} mapWidth={mapDimensions.width} mapHeight={mapDimensions.height} gridSize={1} showBuildingAreas={showClickableAreas} />
          <MapBackButton destination="/explore" size={45} position={{ left: 15, bottom: 60 }} />
        </div>
      </div>
    </div>
  );
